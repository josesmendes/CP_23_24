...
// Host function to copy the value from device to host
//  Function prototypes
...
__device__ void add_Pot(double Pot, double localPot){
    atomicAddDouble(&Pot, localPot);
}

__device__ void add_ma(double *a,double *rij, double f, int i, int j){
    double r0, r1, r2;
    r0 = rij[0] * f;
    r1 = rij[1] * f;
    r2 = rij[2] * f;
    atomicAddDouble(&a[i*3], r0);
    atomicAddDouble(&a[i*3+1], r1);
    atomicAddDouble(&a[i*3+2], r2);
    atomicAddDouble(&a[j*3], -r0);
    atomicAddDouble(&a[j*3+1], -r1);
    atomicAddDouble(&a[j*3+2], -r2);
    //a[i*3]   += rij[0] * f;
    //a[i*3+1] += rij[1] * f;
    //a[i*3+2] += rij[2] * f;
    //a[j*3]   -= rij[0] * f;
...
__global__ void Potential_plus_Accelaration(double *a, double *r, double *Pot)
{
    int i, N, j;
    N = 5000;
    double f, rSqd, term1, term2, epsilon;
    ...
    if (id < N-1 && id >= 0){
        for (j = id + 1; j < N; j ++){
            rSqd = 0.;
            rij[0] = r[id*3]   - r[j*3];
            rij[1] = r[id*3+1] - r[j*3+1];
            rij[2] = r[id*3+2] - r[j*3+2];
            rSqd += rij[0] * rij[0];
            rSqd += rij[1] * rij[1];
            rSqd += rij[2] * rij[2];
...
void launchKerner(){
	...
	checkCUDAError("memcpy h->d");
    cudaMalloc ((void**) &dpot, sizeof(double));
    checkCUDAError("mem allocation");
    cudaMemcpy(dpot, &Pot, sizeof(double), cudaMemcpyHostToDevice);
    checkCUDAError("memcpy h->d");
    //startKernelTime ();
	Potential_plus_Accelaration <<< NUM_THREADS_PER_BLOCK, NUM_BLOCKS >>> (da, dr, dpot);
	//stopKernelTime ();
...
double VelocityVerlet(double dt, int iter, FILE *fp)
{
    ...
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < 3; j++)
        {
            r[i*3+j] += v[i][j] * dt + 0.5 * a[i*3+j] * dt * dt;

            v[i][j] += 0.5 * a[i*3+j] * dt;
        }
